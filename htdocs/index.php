...
$area_sql=mysqli_real_escape_string($link, $area);
$myname_sql=mysqli_real_escape_string($link, $myname);
$toname_sql=mysqli_real_escape_string($link, $toname ?? "");
$subject_sql=mysqli_real_escape_string($link, $subject ?? "");
$text_sql=mysqli_real_escape_string($link, $text);
$myaddr_sql=mysqli_real_escape_string($link, $myaddr);
$toaddr_sql=mysqli_real_escape_string($link, $toaddr ?? "");
$myorigin_sql=mysqli_real_escape_string($link, $myorigin);
$reply_sql=mysqli_real_escape_string($link, $reply ?? "");
$mynode_sql=mysqli_real_escape_string($link, $mynode);

// Area permission: 4 antispam, 3 rw, 2 premoderation, 1 ro, 0 none.
...
//пихаем сообщение в базу, в таблицу outbox.
  if ($permission=="3") { //полный доступ
		$hash=md5(rand());
    mysqli_query($link, "insert into `outbox` set area='$area_sql', fromname='$myname_sql', toname='$toname_sql', subject='$subject_sql', text='$text_sql', fromaddr='$myaddr_sql', toaddr='$toaddr_sql', origin='$myorigin_sql', reply='$reply_sql', date=now(), hash='$hash', sent='0', aprove='1'");
  }
	elseif ($permission=="2") { //доступ с премодерацией
		$hash=md5(rand());
		mysqli_query($link, "insert into `outbox` set area='$area_sql', fromname='$myname_sql', toname='$toname_sql', subject='$subject_sql', text='$text_sql', fromaddr='$myaddr_sql', toaddr='$toaddr_sql', origin='$myorigin_sql', reply='$reply_sql', date=now(), hash='$hash', sent='0', aprove='0'");
  } 
...
=======================================
AREA: $area_sql
FROM: $myname_sql($myaddr_sql)
TO:   $toname_sql($toaddr_sql)
=======================================
$text_sql
* Origin: $myorigin_sql
', fromaddr='$mynode_sql', toaddr='$myaddr_sql', origin='Bad robot', reply='', date=now(), hash='$hash', sent='0', aprove='1'");
	}
